...
export const rejectLeaveProcedure = baseProcedure
  .input(
    z.object({
  ...
  .mutation(async ({ input: { groupId, hash, requestId } }) => {
    // Only the group owner can reject leave requests
    const isOwner = await verifyGroupOwnership(hash, groupId)
    if (!isOwner) {
      throw new TRPCError({
        code: 'UNAUTHORIZED',
        message: 'Only the group owner can reject leave requests.',
      })
    }
    ...
    // Reject: mark the request as rejected (no change to participant)
